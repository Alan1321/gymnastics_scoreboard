...
const Quad = () =>{
    ...
    useEffect(()=>{
        if(currentTeam === 1){
            ...
            receivedData.current['team4']['logo'] = null
        }
    },[currentTeam])
    ...
    const currentTeamName = () =>{
        if(currentTeam === 1){
            ...
            return receivedData.current['team4']['teamName']
        }
    }
    ...
    if(currentTeam === 1){
        ...
        data = fetchPlayers(receivedData.current['team4']['teamName']).map((gymnast)=>gymnast.name)
    }
    ...
    const getDropDownData = (data) =>{
        if(currentTeam === 1){
            ...
            receivedData.current['team4']['lineUpInfo'] = data
        }
        setButtonNext(true)
    }
    ...
    const getPlayerPictures = (data) =>{
        if(currentTeam === 1){
            ...
            receivedData.current['team4']['playerPictures'] = data
        }
    }
    ...
    const getLogo = (data) =>{
        if(currentTeam === 1){
            ...
            receivedData.current['team4']['logo'] = data
        }
    }
    ...
    const sendToParentCheckMeet4 = (data) =>{
        if(data.fix === true){
            setModal(false)
        }
        if(data.next === true){
    ...
    return (
        <div>
            ...
            {currentTeam === 4 &&
                <div>
                    <div style={{display:'flex', alignItems:"center", width:"100%"}}>
                       <h2 style={{textAlign:'center', width:"50%", flexDirection:"row-reverse", display:"flex"}}>Team4 Data</h2>
                        <div style={{marginTop:"1%", display:"flex", flexDirection:"row-reverse", width:'50%'}}>
                            <Button variant="contained" color="success" onClick={checkData} disabled={!buttonNext} style={{marginRight:"45px"}}>
                                Next
                            </Button>
                    ...
                    {modal && <CheckMeetUI  receivedData={receivedData.current['team4']} sendToParent={sendToParentCheckMeet4}/> }
                </div>
            }
        </div>
    )
